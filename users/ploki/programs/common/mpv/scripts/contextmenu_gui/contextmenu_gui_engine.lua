...
local function info(x)
	mp.msg[verbose and "info" or "verbose"](x)
end
local function mpdebug(x)
	mp.msg.info(x)
end -- For printing other debug without verbose
...
local function doMenu(menuList, menuName, x, y, menuPaths, menuIndexes)
	...
	if x == -1 and menuBuilder ~= "tk" then
		x = tostring(mousepos.x)
	end
	if y == -1 and menuBuilder ~= "tk" then
		y = tostring(mousepos.y)
	end
	...
	local function argType(argVal)
		if type(argVal) == "function" then
			argVal = argVal()
		end

		-- Check for nil values and warn here
		if argVal == nil then
			mpdebug("Found a nil value")
		end

		if type(argVal) == "boolean" then
			argVal = tostring(argVal)
		else
			argval = (type(argVal) == "string") and argVal or ""
		end
	...
	if retVal.status ~= 0 then
		...
		if retVal.status == -3 then
			mp.osd_message("Error during initialization of subprocess (Script: " .. scriptfile[menuBuilder] .. ")")
		end
		...
		if menuItem[7] then
			...
			if rebuildMenu == true then
				...
				mp.add_timeout(0, function()
					doMenu(menuList, "context_menu", response.x, response.y, menuPaths, menuIndexes)
				end)
			else
