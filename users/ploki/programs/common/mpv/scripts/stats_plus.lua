...
local function append_perfdata(s, dedicated_page, print_passes)
	...
	s[#s + 1] = format(
		...
		o.font_size * 0.66,
		"（最新/均值/峰值 μs）",
		dedicated_page and "（提示：该页面可使用按键↑↓进行滚动浏览）" or "",
		o.font_size
	)
...
local function add_video_out(s)
	...
	if not r then
		...
		append_resolution(s, { w = scaled_width, h = scaled_height, s = scale }, "分辨率：")
		return
	end
