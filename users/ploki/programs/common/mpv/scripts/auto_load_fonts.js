...
var FONTCONFIG_XML =
  '<?xml version="1.0"?><!DOCTYPE fontconfig SYSTEM "urn:fontconfig:fonts.dtd"><fontconfig>%s</fontconfig>';
...
var commands = (function () {
  function command_native() {
    var args = Array.prototype.slice.call(arguments).filter(function (v) {
      return v !== undefined;
    });
...
function update_options() {
  ...
  options.compatible_dir = format_path(
    commands.expand_path(options.compatible_dir),
  );
  // 如果兼容目录已更改则清理旧目录
...
function write_fonts_conf(fonts_dir, require_exist) {
  ...
  var xml =
    fonts_dir === ""
      ? ""
      : u.string_format(FONTCONFIG_DIR_XML, escape_xml(fonts_dir));
...
(function () {
  mp.add_hook("on_load", 50, function () {
    ...
    var fonts_dir = format_path(
      u.absolute_path(utils.join_path(spaths[0], "fonts")),
    );
    ...
    if (fonts_dir === source_fonts_dir) {
      ...
      msg.info(
        u.string_format(
          "Set fonts dir (compatible_mode): %s (%s)",
          fonts_dir,
          source_fonts_dir,
        ),
      );
