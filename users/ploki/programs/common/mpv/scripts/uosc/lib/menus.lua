...
function create_select_tracklist_type_menu_opener(menu_title, track_type, track_prop, load_command, download_command)
	local function serialize_tracklist(tracklist)
		...
		if download_command then
			items[#items + 1] = {
				title = ulang._dlsub_download,
				bold = true,
				italic = true,
				hint = ulang._dlsub_searchol,
				value = "{download}",
			}
		end
		...
		if track_type == "sub" or track_type == "audio" or track_type == "video" then
			disabled_item = {
				title = ulang._submenu_id_disabled,
				italic = true,
				muted = true,
				hint = "—",
				value = nil,
		...
		for _, track in ipairs(tracklist) do
			if track.type == track_type then
				...
				if track.forced then
					h(ulang._submenu_id_forced)
				end
				if track.default then
					h(ulang._submenu_id_default)
				end
...
function open_file_navigation_menu(directory_path, handle_select, opts)
	...
	if is_root then
		if state.platform == "windows" then
			items[#items + 1] =
				{ title = "..", hint = ulang._submenu_file_browser_item_hint, value = "{drives}", separator = true }
		end
	else
		items[#items + 1] =
			{ title = "..", hint = ulang._submenu_file_browser_item_hint2, value = directory.dirname, separator = true }
	end
...
function open_drives_menu(handle_select, opts)
	...
	if process.status == 0 then
		for _, value in ipairs(split(process.stdout, "\n")) do
			...
			if drive then
				...
				items[#items + 1] = {
					title = drive,
					hint = ulang._submenu_file_browser_item2_hint,
					value = drive_path,
	...
	return Menu:open(
		{
			type = opts.type,
			title = opts.title or ulang._submenu_file_browser_title,
			items = items,
			selected_index = selected_index,
		},
		handle_select
	...
	local function ensure_response_data(success, result, error, check)
		...
		if data.error then
			local message, message_verbose =
				data.message or ulang._dlsub_unknown_err, data.message_verbose or data.message
		...
		mp.command_native_async({
			...
			local items = itable_map(subs, function(sub)
				...
				if sub.attributes.foreign_parts_only then
					hints[#hints + 1] = ulang._dlsub_foreign
				end
				if sub.attributes.hearing_impaired then
					hints[#hints + 1] = ulang._dlsub_hearing
				end
				return {
