...
local function make_xlit(str1, str2)
	...
	tab1:each_with_index(function(v, i)
		strar[v] = tab2[i]
	end)
	...
	local function xlit_pattern(str)
		str = str or ""
		return List(str:split("")):map(function(v)
			return strar[v] or v
		end):concat(), str -- reterun newstr , str
	end
...
local function token(__str)
	...
	local word, chr, substr =
		__str:gsub("[$\\]([0-9acdlpsuwxz])", "%%%1"):gsub("[$\\]U", "%%u"):match("%s*(%w*)(.)(.*[^%s])%s*")
	if substr then
...
local function make_pattern_func(pattern_str)
	...
	if func then
		...
		return nil, true, string.format("(not match /xlit/xform/erase/derive/func/ ): pattern:%s ", pattern_str)
	end
end
