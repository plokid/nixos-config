...
local function conv_pattern(org_text)
	...
	p = p:len() > 0 and "%s" .. conver_rex(p:lower()) .. "[%a%-%.]*%." or "" --  [ ] p [%a]*%."
	return pw, ww, p
end
...
function Word:_initialize(tab)
	if not (type(tab) == "table" and type(tab.word) == "string" and 0 < tab.word:len()) then
		return
	end
...
function Word:match(pw, pn)
	...
	if self.word:lower():match(pw) then
		return pn == "" and true or self.info:match(pn) and true or false
	end
	return false
end
